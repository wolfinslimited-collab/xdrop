...
export default function AdminAgents({ session }: { session: any }) {
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      case 'published': return <Badge className="bg-green-500/15 text-green-400 border-green-500/30 text-[10px] gap-1"><CheckCircle className="w-2.5 h-2.5" />Published</Badge>;
      ...
      default: return <Badge className="bg-amber-500/15 text-amber-400 border-amber-500/30 text-[10px] gap-1"><Clock className="w-2.5 h-2.5" />Draft</Badge>;
    }
  };
  ...
  return (
    <div>
      {/* Filter bar */}
      <div className="flex gap-1 px-6 py-3 border-b border-border bg-secondary/10 flex-wrap">
        {([
          { id: 'all', label: 'All Agents', icon: Bot },
          { id: 'api-connected', label: 'API Connected', icon: Link2 },
        ...
        <span className="ml-auto text-xs text-muted-foreground self-center">{filteredAgents.length} agent{filteredAgents.length !== 1 ? 's' : ''}</span>
      </div>

      {/* Table header */}
      <div className="grid grid-cols-[1fr_100px_90px_110px] px-6 py-2.5 text-[10px] text-muted-foreground uppercase tracking-wider border-b border-border bg-secondary/20">
        <span>Agent</span>
        ...
        {filteredAgents.map((agent: any, i: number) => {
          ...
          return (
            <motion.div
              ...
              className="grid grid-cols-[1fr_100px_90px_110px] items-center px-6 py-3.5 hover:bg-secondary/30 transition-colors"
            >
              ...
              <div className="flex items-center gap-1 flex-wrap">
                {connected ? (
                  <>
                    ...
                    <span className="text-[10px] text-green-400 font-medium">{integrations.length || (manifest ? '✓' : '—')}</span>
                    {integrations.slice(0, 2).map((int: string) => (
                      <Badge key={int} variant="outline" className="text-[9px] px-1.5 py-0 h-4">
                        {int}
                      </Badge>
                    ))}
