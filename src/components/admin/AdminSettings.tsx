...
import { AlertTriangle, Megaphone, ToggleLeft, UserPlus, Save, Loader2 } from 'lucide-react';
...
export default function AdminSettings({ session }: { session: any }) {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      if (map.has('maintenance_mode')) setMaintenance(map.get('maintenance_mode'));
      if (map.has('announcement_banner')) setBanner(map.get('announcement_banner'));
      if (map.has('feature_flags')) setFeatures(map.get('feature_flags'));
      if (map.has('registration')) setRegistration(map.get('registration'));
    } catch {
    ...
    <div className="p-6">
      ...
      {dirty && (
        <motion.div
          ...
          className="flex items-center justify-between p-3 mb-4 rounded-xl bg-accent/10 border border-accent/20"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SettingsCard
          ...
          badge={maintenance.enabled ? <Badge className="bg-accent/10 text-accent border-accent/20 text-[10px]">Active</Badge> : null}
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Label htmlFor="maint-toggle" className="text-sm text-foreground">Enable Maintenance Mode</Label>
              <Switch id="maint-toggle" checked={maintenance.enabled} onCheckedChange={(v) => update(setMaintenance, { enabled: v })} />
            </div>
            <div className="space-y-1.5">
              ...
              <Textarea
                ...
                className="bg-secondary/30 border-border text-sm min-h-[80px] resize-none"
              />
            </div>
          </div>
        </SettingsCard>
        ...
        <SettingsCard
          ...
          badge={banner.enabled ? <Badge className="bg-blue-500/10 text-blue-400 border-blue-500/20 text-[10px]">Live</Badge> : null}
        >
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              <Label htmlFor="banner-toggle" className="text-sm text-foreground">Show Banner</Label>
              <Switch id="banner-toggle" checked={banner.enabled} onCheckedChange={(v) => update(setBanner, { enabled: v })} />
            </div>
            <div className="space-y-1.5">
            ...
            {banner.enabled && banner.message && (
              <div className={`p-3 rounded-lg text-xs ${
                banner.type === 'warning' ? 'bg-accent/10 text-accent border border-accent/20' :
                banner.type === 'error' ? 'bg-destructive/10 text-destructive border border-destructive/20' :
                banner.type === 'success' ? 'bg-success/10 text-success border border-success/20' :
          ...
          <div className="space-y-3">
            {Object.entries(featureLabels).map(([key, { label, desc }]) => (
              <div key={key} className="flex items-center justify-between py-1">
                ...
                <Switch checked={features[key] ?? true} onCheckedChange={() => toggleFeature(key)} />
              </div>
            ))}
          </div>
        </SettingsCard>
        ...
        >
          <div className="space-y-3">
            <div className="flex items-center justify-between py-1">
              ...
              <Switch checked={registration.enabled} onCheckedChange={(v) => update(setRegistration, { enabled: v })} />
            </div>
            <div className="flex items-center justify-between py-1">
              <div>
              ...
              <Switch checked={registration.invite_only} onCheckedChange={(v) => update(setRegistration, { invite_only: v })} />
            </div>
          </div>
        </SettingsCard>
      </div>
...
function SettingsCard({ icon: Icon, title, description, badge, children }: {
  icon: any; title: string; description: string; badge?: React.ReactNode; children: React.ReactNode;
}) {
  return (
    <motion.div
      initial={{ opacity: 0, y: 8 }}
